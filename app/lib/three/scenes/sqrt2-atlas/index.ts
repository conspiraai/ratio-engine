...
export type Sqrt2AtlasSceneOptions = {
  squareSize: number;
  depth: number;
  layerCount: number;
};
...
export const createScene = (
  ...
  options: Sqrt2AtlasSceneOptions,
): CreateSceneResult => {
  const { canvas, width, height, dpr, isMobile } = params;
  const { squareSize, depth, layerCount } = options;
  ...
  const zStep = depth / Math.max(1, layerCount - 1);

  for (let i = 0; i < layerCount; i += 1) {
    const size = squareSize * (1 - i * 0.1);
    const geometry = new THREE.BufferGeometry().setFromPoints([
      new THREE.Vector3(-size / 2, -size / 2, i * zStep),
      new THREE.Vector3(size / 2, -size / 2, i * zStep),
      new THREE.Vector3(size / 2, size / 2, i * zStep),
      new THREE.Vector3(-size / 2, size / 2, i * zStep),
    ]);
  ...
  const planeGeometry = new THREE.PlaneGeometry(squareSize * 1.1, squareSize * 1.1);
  ...
  const pane = new THREE.Mesh(planeGeometry, planeMaterial);
  pane.position.z = -0.2;

  root.add(frameGroup, diagonal, pane);
  ...
  const update = (delta: number) => {
    ...
    frameGroup.rotation.z = prefersReducedMotion ? 0 : Math.sin(elapsed * 0.3) * 0.04;
